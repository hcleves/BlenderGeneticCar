...
def gera_matrix_ganhos_2(genes,pontuacoes):
    matrix = numpy.asarray(genes[0])
    for i in range(1,len(genes)):
        matrix = numpy.vstack((matrix,numpy.asarray(genes[i])))
        
    matrix = numpy.hstack((matrix, numpy.asarray(pontuacoes).reshape(len(genes),1)))
    matrix = numpy.hstack((matrix, numpy.asarray(gera_lista_indices(len(genes))).reshape(len(genes),1)))
    return matrix
...
for i in range(0,len(obj['lista_carros'])):
    carro = obj['lista_carros'][i]
    
    if carro['pontuacao']>max_pontuacao:
        max_pontuacao = carro['pontuacao']*1;
        scene.objects['Camera'].actuators[0].object = carro;
        obj['pi'] = max_pontuacao;
        
    if carro['roubou']:
        obj['lista_genes'].append(numpy.copy(carro['gene']))
        obj['lista_pontuacao'].append(numpy.copy(carro['pontuacao']))
        paraDeletar.append(i)
        continue
    if obj['tempo']>2:
        if numpy.linalg.norm(carro.getLinearVelocity())<0.002 or carro['pontuacao']<obj['tempo']*10 or carro['pontuacao']<100:
            obj['lista_genes'].append(carro['gene'])
            obj['lista_pontuacao'].append(carro['pontuacao'])
            paraDeletar.append(i)


for pontuacao in obj['lista_pontuacao']:
    if pontuacao > max_pontuacao:
        max_pontuacao = pontuacao;
        

for i in range(len(paraDeletar)-1,-1,-1):
    obj['lista_carros'][paraDeletar[i]].endObject();
    obj['lista_carros'].pop(paraDeletar[i]);
    obj['lista_cenarios'][paraDeletar[i]].endObject();
    obj['lista_cenarios'].pop(paraDeletar[i]);
    
nova_geracao = False;

if len(obj['lista_carros']) == 0:
    nova_geracao = True
...
if nova_geracao:
    ...
    matrix_desordenada = gera_matrix_ganhos_2(obj['lista_genes'],obj['lista_pontuacao'])
